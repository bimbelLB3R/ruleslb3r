...
const ContactForm = ({ sheetdata }) => {
  ...
  useEffect(() => {
    ...
    sheetdata.forEach((index) => {
      ...
      const savedValue = localStorage.getItem(`kuanti${index[0]}`);
  ...
  const checkNisn = async (nisn) => {
    ...
    await doc.loadInfo();
    const sheet5 = doc.sheetsById[SHEET_ID5];
    const sheet6 = doc.sheetsById[SHEET_ID6];
    const rows = await sheet5.getRows();
    const rowToDelete = rows.find((row) => row.nisn === nisn);

    const rows4 = await sheet6.getRows();
  ...
  const submitForm = async (e, sheet5) => {
    e.preventDefault();
    setIsButtonDisabled(true);
    // cek ricek
    ...
    if (isValid) {
      const nisnAda = await checkNisn(form.nisn, sheet5);
      if (nisnAda) {
      ...
      router.push('/form/outputkuantitatif');
    } else {
      Swal.fire({
        title: 'Error',
        text: errorMessage,
                  ...
                  <div className=" border-dashed border-l-2 border-yellow-900">
                    ...
                    <div className="pr-4 pl-4">
                      <Radio.Group
                        ...
                        value={selectedValues[`kuanti${item[0]}`]}
                        name={`kuanti${item[0]}`}>
...
export async function getServerSideProps() {
  const req = await fetch('https://ruleslb3r.vercel.app/api/soalkuantitatif');
