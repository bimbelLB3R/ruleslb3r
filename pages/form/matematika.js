...
const ContactForm = ({ sheetdata }) => {
  ...
  useEffect(() => {
    ...
    sheetdata.forEach((index) => {
      ...
      const savedValue = localStorage.getItem(`mtk${index[0]}`);
  ...
  const checkNisn = async (nisn) => {
    ...
    await doc.loadInfo();
    const sheet7 = doc.sheetsById[SHEET_ID7];
    const sheet8 = doc.sheetsById[SHEET_ID8];
    const rows = await sheet7.getRows();
    const rowToDelete = rows.find((row) => row.nisn === nisn);

    const rows4 = await sheet8.getRows();
  ...
  const submitForm = async (e, sheet7) => {
    e.preventDefault();
    setIsButtonDisabled(true);
    // cek ricek
    ...
    if (isValid) {
      const nisnAda = await checkNisn(form.nisn, sheet7);
      if (nisnAda) {
      ...
      router.push('/form/outputmatematika');
    } else {
      Swal.fire({
        title: 'Error',
        text: errorMessage,
  ...
  return (
    <div>
      <Head>
        <title>PENALARAN MATEMATIKA|SNBT</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/image/logolb3r.png" />
      </Head>
      {/* referensi https://sylhare.github.io/2022/03/23/Katex.html */}
      <link
        rel="stylesheet"
                  ...
                  <div className=" border-dashed border-l-2 border-yellow-900">
                    ...
                    <div className="pr-4 pl-4">
                      <Radio.Group
                        ...
                        value={selectedValues[`mtk${item[0]}`]}
                        name={`mtk${item[0]}`}>
...
export async function getServerSideProps() {
  const req = await fetch('https://ruleslb3r.vercel.app/api/soalmatematika');
